...
func parseTemplateMetadata(blueprintVars *[]byte, templatePath string, blueprintRepository *BlueprintContext, isLocal bool) (*BlueprintConfig, error) {
	...
	templateConfigs, err := yamlDoc.parseFiles(isLocal)
	if err != nil {
		return nil, err
	}
...
func (blueprintDoc *BlueprintYaml) parseFiles(isLocal bool) ([]TemplateConfig, error) {
	var files []File
	...
	for _, m := range files {
		...
		if isLocal {
			...
			templateConfig.Path = AdjustPathSeperatorIfNeeded(templateConfig.Path)
		}
